...
#define ITER 2       // number of BP iterations at each scale
#define ITER0 5
...
#define TWIDTH 64	// Width of tile
#define THEIGHT 64	// Height of tile
	...
	for (int t1 = 0; t1 < ITER; t1++) {
		...
		for (int row = 1; row < numHeightTiles-1; row++) {
			for (int col = 1; col < numWidthTiles-1; col++) {
				...
				for (int t0 = 0; t0 < ITER0; t0++) {
					...
					for (int y = 0; y < imRef(hTile, row, 0); y++) {
						for (int x = ((y + t0) % 2); x < imRef(wTile, col, 0); x += 2) {
							...
							if (y == 0 && x == 0) {
								//std::cout << "height: " << imRef(hTile, row, 0) << "\n";
							...
							if (t0 == 0) {
								...
								case '1':
									msg1(imRef(d, x, y - 1), imRef(l, x + 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(lBound, col, cumTileHeight + y));

									msg1(imRef(d, x, y - 1), imRef(l, x + 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(dBound, cumTileWidth + x, row));
									break;
								case '2':
									msg(imRef(d, x, y - 1), imRef(l, x + 1, y), imRef(r, x - 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(dBound, cumTileWidth + x, row));
								...
								case '3':
									msg1(imRef(d, x, y - 1), imRef(r, x - 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(rBound, col, cumTileHeight + y));
								...
								case '7':
									...
									msg1(imRef(u, x, y + 1), imRef(l, x + 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(uBound, cumTileWidth + x, row));
									break;
								case '8':
									msg(imRef(u, x, y + 1), imRef(l, x + 1, y), imRef(r, x - 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(uBound, cumTileWidth + x, row));
								...
								switch (node) {
								case '1':
									msg2(imRef(l, x + 1, y), imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(u, x, y));

									msg2(imRef(d, x, y - 1), imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(r, x, y));
									break;
								case '2':
									...
									break;
								case '3':
									msg2(imRef(r, x - 1, y), imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(u, x, y));

									msg2(imRef(d, x, y - 1), imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(l, x, y));
									break;
								case '4':
									msg1(imRef(u, x, y + 1), imRef(l, x + 1, y),
										imRef(data, cumTileWidth + x, cumTileHeight + y), imRef(u, x, y));
				...
				for (int y = 1; y < imRef(hTile, row, 0)-1; y++) {
					for (int x = 1; x < imRef(wTile, col, 0)-1; x ++) {
						int best = 0;
						float best_val = INF;
						for (int value = 0; value < VALUES; value++) {
							float val =
