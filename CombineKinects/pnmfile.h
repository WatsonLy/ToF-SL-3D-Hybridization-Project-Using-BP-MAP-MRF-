...
*/
static image<uint16_t> *loadPGM(char *name) {
  char buf[BUF_SIZE];
  ...
  /* read data */
  image<uint16_t> *im = new image<uint16_t>(width, height);
  file.read((char *)imPtr(im, 0, 0), width * height * sizeof(uint16_t));

  return im;
}

static void savePGM(image<uint16_t> *im, char *name) {
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);

  file << "P5\n" << width << " " << height << "\n" << UCHAR_MAX << "\n";
  file.write((char *)imPtr(im, 0, 0), width * height * sizeof(uint16_t));
}
